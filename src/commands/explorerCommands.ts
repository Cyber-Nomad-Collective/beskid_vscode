...
import {
	...
	resolveFocusTarget,
} from "../workspace/resolveFocusTarget.js";
...
async function revealInProjectsTree(
	...
	if (projectUri) {
		await views.projectsTreeView.reveal(
			projectsTree.getRevealTarget(projectUri),
			{
				select: true,
				focus: false,
				expand: true,
		...
		vscode.commands.registerCommand("beskid.selectProject", async () => {
			...
			if (files.length === 0) {
				void vscode.window.showWarningMessage(
					"No .bproj files found in workspace.",
				);
				return;
			}
...
export function registerDeferredExplorerCommands(
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("beskid.revealInProjectsTree", async () => {
			await revealInProjectsTree(
				deps.views,
				deps.projectsTree,
				deps.focus.getFocusedProject(),
			);
		}),
