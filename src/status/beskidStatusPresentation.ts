...
export function deriveBeskidStatusPresentation(
	snapshot: BeskidStatusSnapshot,
): {
	text: string;
	tooltipLines: string[];
} {
	...
	if (snapshot.lspClientRunning) {
		text =
			parts.length > 0 ? `$(zap) Beskid: ${joined}` : "$(zap) Beskid LSP: Running";
	} else if (transitional && snapshot.runtimePhase) {
		const label =
			RUNTIME_PHASE_LABEL[snapshot.runtimePhase] ?? snapshot.runtimePhase;
		const icon =
