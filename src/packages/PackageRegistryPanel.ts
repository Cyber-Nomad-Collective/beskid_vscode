...
import { renderPackageRegistryHtml, type RegistryPanelState } from "./packageRegistryHtml.js";
...
export class PackageRegistryPanel extends WebviewPanelHost {
  ...
  register(context: vscode.ExtensionContext): void {
    ...
    context.subscriptions.push(
      ...
      vscode.commands.registerCommand("beskid.packages.registrySelect", (name: string) => {
        this.open();
  ...
  private async handleMessage(message: unknown): Promise<void> {
    ...
    const msg = message as { type: string; query?: string; name?: string; url?: string; text?: string; command?: string };
    switch (msg.type) {
      case "search":
        await this.runSearch(msg.query ?? "");
        break;
      case "refresh":
        await (this.state.query.trim() ? this.runSearch(this.state.query) : this.loadBrowse());
        break;
      case "select":
        if (msg.name) {
  ...
  private async loadBrowse(): Promise<void> {
    ...
    this.state = { ...this.state, loading: true, error: undefined, registryBaseUrl: base, query: "" };
    this.render();
    this.reportActivity("search", true, "Loading catalog…");
    const result = await this.pckg.list(50);
    this.reportActivity("search", false);
    if (!result.ok) {
      this.state = { ...this.state, loading: false, error: result.error, rows: [] };
    } else {
  ...
  private async runSearch(query: string): Promise<void> {
    const base = await this.pckg.resolveRegistryBaseUrl();
    this.state = { ...this.state, query, loading: true, error: undefined, registryBaseUrl: base };
    this.render();
    this.reportActivity("search", true, query ? `Searching “${query}”…` : "Loading catalog…");
