...
export function appendDependencyToProjectManifest(
  ...
  if (new RegExp(`dependency\\s+"${input.name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}"`, "i").test(text)) {
    throw new Error(`Dependency "${input.name}" already exists in project manifest.`);
  }
...
export async function promptAndAddDependency(projectUri: vscode.Uri): Promise<boolean> {
  ...
  try {
    ...
    void vscode.window.showInformationMessage(`Added dependency "${name.trim()}" to project manifest.`);
    return true;
  } catch (e) {
