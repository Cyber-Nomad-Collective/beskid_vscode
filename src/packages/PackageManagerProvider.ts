...
export function formatPckgConnectionLabel(
	status: PckgConnectionStatus,
): string {
	const url =
		status.baseUrl || status.workspaceDefaultRegistryUrl || "not configured";
	if (status.connected) {
...
export class PackageManagerProvider
	implements vscode.TreeDataProvider<PackageTreeItem>
{
	...
	private async getThisProjectChildren(): Promise<PackageTreeItem[]> {
		...
		if (!projectUri) {
			return [
				new PackageTreeItem(
					"info",
					"ThisProject",
					"Focus a project first.",
					vscode.TreeItemCollapsibleState.None,
				),
			];
		}
		const depsOutcome = await this.deps.lspApi.getProjectDependencies(
			projectUri.toString(),
		);
		if (!depsOutcome.ok) {
			return [
		...
		for (const dep of deps.declared) {
			...
			const item = new PackageTreeItem(
				...
				undefined,
				{
					...
					materializedPath: deps.locked.find((l) => l.name === dep.name)
						?.materializedPath,
					label,
				},
			);
			...
			if (!lockedNames.has(dep.name.toLowerCase())) {
				item.description = [item.description, "not locked"]
					.filter(Boolean)
					.join(" · ");
			}
		...
		for (const dep of deps.locked) {
			if (
				deps.declared.some((d) => d.name.toLowerCase() === dep.name.toLowerCase())
			) {
				continue;
			}
			const label = dep.version ? `${dep.name}@${dep.version}` : dep.name;
