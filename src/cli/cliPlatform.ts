...
export function resolveCliPlatformAsset(
  ...
  if (platform === "win32" && normalizedArch === "amd64") {
    return { releaseAsset: "beskid-windows-amd64.exe", installFileName: "beskid.exe" };
  }
  return undefined;
}
...
export function defaultCliInstallPath(
  ...
  if (!asset) {
    return join(defaultCliInstallDir(), platform === "win32" ? "beskid.exe" : "beskid");
  }
  return join(defaultCliInstallDir(), asset.installFileName);
}

export function cliReleaseDownloadUrl(releaseTag: string, assetName: string): string {
