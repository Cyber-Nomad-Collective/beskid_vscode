...
export type BeskidCliSubcommand =
	| "fetch"
	| "lock"
	| "build"
	| "test"
...
export function focusedProjectCwd(
		...
		child.on("close", (code) => {
			...
			if (exitCode !== 0) {
				...
				void vscode.window.showInformationMessage(
					`Beskid ${options.subcommand} completed.`,
				);
			}
			resolve(exitCode);
		});
		child.on("error", (err) => {
			...
			if (errno.code === "ENOENT") {
				...
				void vscode.window.showErrorMessage(
					`Failed to run Beskid CLI: ${err.message}`,
				);
			}
			resolve(1);
		});
...
export async function resolveRegistryBaseUrl(
	lspGetSummary: (
		workspaceUri: string,
	) => Promise<{ registries: Record<string, string> } | undefined>,
