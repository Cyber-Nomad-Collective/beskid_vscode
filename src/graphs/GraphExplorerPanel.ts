...
import {
	type GraphPanelViewState,
	renderGraphPanelHtml,
} from "./graphPanelHtml.js";
		...
		if (kind === "workspace") {
			if (!isWorkspaceManifestUri(focused.fsPath)) {
				void vscode.window.showWarningMessage(
					"Open a .bws workspace manifest for the workspace graph.",
				);
				return;
			}
			...
			void vscode.window.showWarningMessage(
				"Focus a .bproj project manifest for this graph kind.",
			);
			return;
			...
			void vscode.window.showWarningMessage(
				"Focus a Beskid .bproj manifest first.",
			);
			return;
		}
	...
	async refresh(): Promise<void> {
		...
		if (!outcome.ok) {
			void vscode.window.showErrorMessage(
				`Failed to load graph from language server: ${outcome.error}`,
			);
			return;
		}
		...
		const panelUri = webview.asWebviewUri(
			vscode.Uri.joinPath(this.extensionUri, "media", "graph", "panel.js"),
		);
		this.postHtml(
			renderGraphPanelHtml(
				webview.cspSource,
