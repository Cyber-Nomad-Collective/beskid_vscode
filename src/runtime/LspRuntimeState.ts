...
function readSettingsFlags(): LspSettingsFlags {
	...
	return {
		...
		autoInstallOnLaunch: beskid.get<boolean>(
			"toolchain.autoInstallOnLaunch",
			true,
		),
		autoSelectFromEditor: beskid.get<boolean>(
			"project.autoSelectFromEditor",
			true,
		),
		logLevel: lsp.get<string>("log.level", "info"),
		logServerOutput: lsp.get<boolean>("log.serverOutput", true),
		pckgBaseUrl: beskid.get<string>(
			"pckg.baseUrl",
...
export class LspRuntimeState {
	...
	setClientRunning(running: boolean): void {
		...
		const next = phaseAfterSetClientRunning(
			this.phase,
			running,
			this.clientStartedOnce,
		);
		if (next) {
			this.phase = next;
