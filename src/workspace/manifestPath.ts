...
function discoverProjectManifestInDir(dir: string): string | undefined {
	...
	try {
		for (const entry of readdirSync(dir, { withFileTypes: true })) {
			if (
				entry.isFile() &&
				entry.name.toLowerCase().endsWith(PROJECT_MANIFEST_EXTENSION)
			) {
...
export function discoverProjectFileFromPath(
	filePath: string,
): string | undefined {
...
export function discoverWorkspaceManifestInDir(
	dir: string,
): string | undefined {
	const matches: string[] = [];
	try {
		for (const entry of readdirSync(dir, { withFileTypes: true })) {
			if (
				entry.isFile() &&
				entry.name.toLowerCase().endsWith(WORKSPACE_MANIFEST_EXTENSION)
			) {
				matches.push(join(dir, entry.name));
			}
		}
