...
export type FocusTargetInput = vscode.Uri | string | ProjectsTreeItem | undefined;

/** Resolve a focus target to a `.bproj` manifest URI, if possible. */
export function resolveFocusTarget(input: FocusTargetInput): vscode.Uri | undefined {
  if (input && typeof input === "object" && "scheme" in input && "fsPath" in input) {
