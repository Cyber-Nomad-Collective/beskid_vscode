...
describe("registerViews", () => {
	test("registers core tree views with createTreeView", async () => {
		...
		expect(mockVscode.module.window.createTreeView).toHaveBeenCalledTimes(
			BESKID_TREE_VIEW_IDS.length,
		);
	...
	test("registers debug tree when debug.enabled is true", async () => {
		...
		const registered = registerViews(
			{ subscriptions: mockVscode.subscriptions } as never,
			deps as never,
		);
