...
describe("resolveFocusManifestPath", () => {
  ...
  test("resolves bproj file path string", () => {
    expect(resolveFocusManifestPath("/repo/apps/demo/demo.bproj")).toBe("/repo/apps/demo/demo.bproj");
  });
...
`,
      ...
      expect(path?.replaceAll("\\", "/")).toBe(join(root, "demo.bproj").replaceAll("\\", "/"));
    } finally {
  ...
  test("normalizes backslashes in bproj paths", () => {
    expect(resolveFocusManifestPath("C:\\repo\\demo.bproj")).toBe("C:/repo/demo.bproj");
  });
});
