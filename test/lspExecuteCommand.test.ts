...
import { LspCommandError, unwrapLspResult } from "../src/lsp/lspExecuteCommand.js";

describe("LspCommandError", () => {
  test("includes command name and nested error message", () => {
    const error = new LspCommandError("beskid.listWorkspaces", new Error("server offline"));
...
describe("unwrapLspResult", () => {
  test("returns value for successful commands", () => {
    expect(unwrapLspResult({ ok: true, value: { workspaces: [] } })).toEqual({ workspaces: [] });
  });
