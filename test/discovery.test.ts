...
`,
      ...
      expect(discovered?.replaceAll("\\", "/")).toBe(join(root, "demo.bproj").replaceAll("\\", "/"));
    } finally {
  ...
  test("returns undefined when directory has multiple bproj files", () => {
    ...
    try {
      ...
      expect(discoverProjectFileFromPath(join(root, "src/main.bd"))).toBeUndefined();
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
...
`,
      ...
      expect(discoverWorkspaceManifestInDir(root)).toBe(join(root, "Demo.bws").replaceAll("\\", "/"));
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
