...
function createMockVscode() {
  ...
  const registerCommand = mock((id: string, handler: () => unknown | Promise<unknown>) => {
...
describe("BeskidModalPanel status dashboard", () => {
  ...
  test("register wires webview provider, commands, and runtime subscription", async () => {
    ...
    const { panel } = { panel: new BeskidModalPanel(createRuntimeMock() as never, "0.2.0") };
    ...
    expect(mockVscode.registerCommand).toHaveBeenCalledWith("beskid.modal.open", expect.any(Function));
  ...
  test("registered modal commands focus the panel webview", async () => {
    const { BeskidModalPanel, BESKID_DASHBOARD_VIEW_ID } = await loadModalPanelModule();
    const panel = new BeskidModalPanel(createRuntimeMock() as never, "0.2.0");
    panel.register({ subscriptions: mockVscode.subscriptions } as never);
    panel.resolveWebviewView(mockVscode.createWebviewView() as never, {} as never, {} as never);
    ...
    expect(mockVscode.executeCommand).toHaveBeenCalledWith(`${BESKID_DASHBOARD_VIEW_ID}.focus`);
  });

  test("resolveWebviewView enables scripts and renders initial dashboard HTML", async () => {
    const { BeskidModalPanel } = await loadModalPanelModule();
    const panel = new BeskidModalPanel(
      createRuntimeMock({ phase: "running", cliVersion: "1.2.3", lspVersion: "4.5.6" }) as never,
      "0.2.0",
    );
