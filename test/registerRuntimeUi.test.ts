...
function createRuntimeMock(initial: Partial<LspRuntimeSnapshot> = {}) {
	const snapshot = testRuntimeSnapshot(initial);
...
describe("registerRuntimeUi status dashboard", () => {
	test("registers BeskidModalPanel webview provider and dashboard commands", async () => {
		...
		const registerWebviewViewProvider = mock(() => ({
			dispose: () => undefined,
		}));
		...
		const { registerRuntimeUi } = await import(
			"../src/activation/registerRuntimeUi.js"
		);
		const runtime = createRuntimeMock();
		const handles = registerRuntimeUi(
