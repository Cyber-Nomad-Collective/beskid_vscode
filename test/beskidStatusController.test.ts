...
function createMockRuntime(initial: Partial<LspRuntimeSnapshot> = {}): LspRuntimeState {
  ...
  return {
    ...
    applyLspNotification: (params: BeskidStatusParams) => {
      const outcome = applyWorkspaceScanNotification(snapshot.phase, snapshot.scan, params);
      if (outcome) {
        snapshot = {
          ...snapshot,
          phase: outcome.phase,
    ...
    setPckgActivity: (phase: BeskidActivityPhase, active: boolean, detail?: string) => {
      if (active) {
        snapshot = {
          ...snapshot,
